#!/usr/bin/env python3
"""
Create Simple Test Flow for Dialogflow CX Agent
Flow: Greeting → Confirmation → End
"""
import os
import sys
import json
...
def create_test_flow():
    """Create a simple test conversation flow."""
    
    agent_name = load_agent_name()
    print(f"Creating test flow for agent: {agent_name}")
    ...
    flow = dialogflow_cx.Flow(
        ...
        description="Simple test conversation for voice calling system",
    )
    ...
    greeting_text = "Hi, this is Paul from Fortinet testing the voice system. Can you hear me okay?"
    ...
    # Get or create Confirmation Page
    if "confirmation" in page_map:
        confirmation_page_name = page_map["confirmation"]
        print(f"   ✓ Using existing: confirmation")
    else:
        confirmation_page = dialogflow_cx.Page(
            display_name="confirmation",
            form=dialogflow_cx.Form(
                parameters=[
                    dialogflow_cx.Form.Parameter(
                        display_name="user_response",
        ...
        confirmation_page_name = get_or_create_page(pages_client, flow_name, confirmation_page, "confirmation")
    
    # Get or create End Page
    if "end-call" in page_map:
        end_page_name = page_map["end-call"]
        print(f"   ✓ Using existing: end-call")
    elif "end" in page_map:
        ...
        end_page = dialogflow_cx.Page(
            ...
            entry_fulfillment=dialogflow_cx.Fulfillment(
                messages=[
                    dialogflow_cx.ResponseMessage(
                        text=dialogflow_cx.ResponseMessage.Text(
                            text=["Great! Test successful. Talk soon."]
                        )
                    )
                ]
            ),
        )
        end_page_name = get_or_create_page(pages_client, flow_name, end_page, "end-call")
    ...
    try:
        greeting_page_updated = dialogflow_cx.Page(
            ...
            transition_routes=[
                dialogflow_cx.TransitionRoute(
                    condition="true",
                    target_page=confirmation_page_name,
                )
            ],
        )
        update_mask = field_mask_pb2.FieldMask(paths=["transition_routes"])
        pages_client.update_page(page=greeting_page_updated, update_mask=update_mask)
        print(f"   ✓ Greeting → Confirmation")
    except Exception as e:
        print(f"   ⚠ Greeting transition: {e}")
    
    # Confirmation → End
    try:
        confirmation_page_updated = dialogflow_cx.Page(
            name=confirmation_page_name,
            transition_routes=[
                dialogflow_cx.TransitionRoute(
        ...
        update_mask = field_mask_pb2.FieldMask(paths=["transition_routes"])
        pages_client.update_page(page=confirmation_page_updated, update_mask=update_mask)
        print(f"   ✓ Confirmation → End")
    except Exception as e:
        print(f"   ⚠ Confirmation transition: {e}")
    
    # Configure End page to terminate session
    print(f"\n4. Configuring call termination...")
    try:
        end_page_with_termination = dialogflow_cx.Page(
            name=end_page_name,
            entry_fulfillment=dialogflow_cx.Fulfillment(
                messages=[
                    dialogflow_cx.ResponseMessage(
        ...
        update_mask = field_mask_pb2.FieldMask(paths=["entry_fulfillment"])
        pages_client.update_page(page=end_page_with_termination, update_mask=update_mask)
        print(f"   ✓ End page will hang up call")
    except Exception as e:
        print(f"   ⚠ End page config: {e}")
    
    # Set as agent's start flow
    print(f"\n5. Setting as default flow...")
    try:
    ...
    flow_config = {
        ...
        "pages": {
            ...
            "confirmation": confirmation_page_name,
            "end": end_page_name,
        },
        "tts_voice": TTS_VOICE,
        "conversation": {
            "greeting": greeting_text,
            "ending": "Great! Test successful. Talk soon.",
        }
    }
    
    flow_config_file = os.path.join(config_dir, "test-flow.json")
    with open(flow_config_file, "w") as f:
        json.dump(flow_config, f, indent=2)
    print(f"   ✓ Config saved: test-flow.json")
    
    # Summary
    print(f"\n{'='*70}")
    print(f"✅ TEST FLOW READY!")
    print(f"{'='*70}")
    print(f"\nConversation:")
    print(f"  1. '{greeting_text}'")
    print(f"  2. [Listen for response]")
    print(f"  3. 'Great! Test successful. Talk soon.' [Hang up]")
    print(f"\n🎤 Voice: {TTS_VOICE}")
    print(f"{'='*70}")
